...
try:
    ...
    # ── HEAL THE VECTORIZER ─────────────────────────────────────
    # If the vectorizer lost its dictionary (built=False), we 
    # inject the JSON list right back into it!
    try:
        with open("smishguard_vocab.json", "r", encoding="utf-8") as f:
            vocab_list = json.load(f)
            
        # Ensure it's a list (Keras TextVectorization expects a list)
        if isinstance(vocab_list, dict):
            # In case it saved as a dict like {"word": 2}, reconstruct the list
            # Keras indices: 0="", 1="[UNK]"
            max_idx = max(vocab_list.values()) if vocab_list else 1
            temp_list = [""] * (max_idx + 1)
            temp_list[1] = "[UNK]"
            for w, i in vocab_list.items():
                temp_list[i] = w
            vocab_list = temp_list
            
        model.layers[0].set_vocabulary(vocab_list)
        log.info(f"✅ Vocabulary restored! Tokens: {len(vocab_list):,}")
    except FileNotFoundError:
        log.warning("⚠️  smishguard_vocab.json not found — AI may predict 0.0 for everything.")
    except Exception as ve:
        log.error(f"❌ Failed to heal vocabulary: {ve}")
...
def analyze_url(raw_text: str, url: str) -> tuple[float, list[str], bool, bool]:
    ...
    Signals (priority order):
      ...
      6. Suspicious TLD alone  → +45% [Fix 4] (was 35, catches friendly+malicious-TLD)
    """
    risk        = 0.0
    logs        = []
    is_critical = False
...
async def predict(request: SMSRequest):
    try:
        ...
        try:
            if model:
                # The healed model automatically handles the string
                # no matter what platform it is running on!
                ai_prob = float(
...
# 1. Define the Expected JSON Payload for Feedback
class FeedbackRequest(BaseModel):
    text: str
    user_corrected_label: int  # 1 for Smishing, 0 for Ham
    model_score: float

# 2. Setup the Cloud Sync Scheduler for Hugging Face Datasets
# This creates a local folder to buffer the CSV before pushing to HF
feedback_folder = Path("feedback_data")
feedback_folder.mkdir(exist_ok=True)
feedback_file = feedback_folder / "reports.csv"

# Configure the scheduler (Syncs to HF every 5 minutes)
# ⚠️ IMPORTANT: Change "your-username/smishguard-feedback" to your actual HF repo
hf_repo_id = os.getenv("HF_DATASET_REPO", "chrisbinsibi/smishguard-feedback")
...
# 3. The Report Endpoint
@app.post("/report")
async def log_feedback(request: FeedbackRequest):
    try:
        # Sanitize text to prevent CSV corruption (remove newlines, escape quotes)
        safe_text = request.text.replace("\n", " ").replace('"', '""')
        new_row = f'"{safe_text}",{request.user_corrected_label},{request.model_score}\n'
        
        # Write safely using the scheduler's thread-safe lock
        with scheduler.lock:
            with feedback_file.open("a", encoding="utf-8") as f:
                # Write the header if the file is completely empty
                if feedback_file.stat().st_size == 0:
                    f.write("text,label,original_score\n")
                f.write(new_row)
        
        log.info(f"✅ Feedback logged - Label: {request.user_corrected_label} | Score: {request.model_score}")
        
        return {
            "status": "success", 
            "message": "Feedback safely queued for cloud sync.",
            "recorded_label": request.user_corrected_label
        }
